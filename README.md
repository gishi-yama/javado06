# Java Do #06

JAX-RS（Jersy）の初心者向けハンズオンです。

- [1. JAX-RS（Jersy）のMavenプロジェクトを作成しよう](./doc/01.md)
- [2. MavenプロジェクトをIDEで読み込もう](./doc/02.md)
- [3. pom.xmlのJavaのバージョンを上げよう](./doc/03.md)
- [4. 起動の確認をしよう](./doc/04.md)
- [5. index.htmlを表示しよう](./doc/05.md)
- [6. Resourceを作ろう](./doc/06.md)
- [7.【演習】かけ算をするResourceを追加しよう](./doc/07.md)
- [8. ResourceでXMLをレスポンスしよう](./doc/08.md)
- [9. ResourceでJSONをレスポンスしよう](./doc/09.md)
- [10. 商品の一連の操作を実装しよう（前準備）](./doc/10.md)
- [11. Rest Client を準備しよう](./doc/11.md)
- [12. 商品を取得しよう](./doc/12.md)
- [13. 商品を登録しよう](./doc/13.md)
- [14. 商品を更新しよう](./doc/14.md)
- [15. 商品を削除しよう](./doc/15.md)
- [16. HTMLからAPIをよびだそう](./doc/16.md)


# 参考

- [JAX-RSを始める #javaee 裏紙](http://backpaper0.github.io/2014/12/01/javaee_advent_calendar_2014.html)
- [JAX-RS入門および実践](http://backpaper0.github.io/ghosts/jaxrs-getting-started-and-practice.html#1)
- [JavaEEでもできる！JAX-RSでお手軽REST開発](https://cloudear.jp/blog/?p=2005)
